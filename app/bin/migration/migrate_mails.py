...
for res in results:
    ...
    cur2.execute(
        "INSERT INTO mails_mail (id, subject, sent, due, user_id) VALUES (%s, %s, %s, %s, %s)",
        (mail["id"], mail["subject"], mail["sent"], mail["due"], mail["user_id"]),
    )
    for r in recipients:
        cur2.execute(
            "INSERT INTO mails_recipient (mail_id, name, email) VALUES (%s, %s, %s)",
            (mail["id"], r["name"], r["email"]),
        )
