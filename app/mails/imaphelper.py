...
class IMAPMessage(object):
    ...
    def from_dbid(cls, dbid, imap_conn):
        ...
        results, data = imap_conn.uid(
            "search", None, '(KEYWORD "MAILDELAY-%d")' % int(dbid)
        )
    ...
    def _get_msg_from_imap(self):
        ...
        if "utf-8" not in msg.get_charsets() and msg.get_charsets()[0]:
            msg = email.message_from_string(
                str(data[0][1].decode(msg.get_charsets()[0]).encode("utf-8"))
            )
        return msg
    ...
    def get_subject(self):
        ...
        try:
            ...
            header_texts = [self._fix_header_encoding(hdr) for hdr in subjects_raw]
        ...
        for key in self.recipient_headers:
            try:
                ...
                for recipient in recipient_list:
                    r = {"name": recipient[0], "email": recipient[1]}
